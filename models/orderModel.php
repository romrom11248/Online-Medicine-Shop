...
function createOrder($userId, $total, $address, $status, $method){
    ...
    mysqli_stmt_bind_param($stmt, "idsss", $userId, $total, $address, $status, $method);
...
function createOrderItem($orderId, $medicineId, $quantity, $price){
    $con = getConnection();
    $sql = "INSERT INTO order_items(order_id, medicine_id, quantity, unit_price)
            VALUES(?, ?, ?, ?)";
    $stmt = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($stmt, "iiid", $orderId, $medicineId, $quantity, $price);
